...
export default function ProductCard({ product }) {
  ...
  return (
    <div
      className="group bg-white border border-gray-100 rounded-2xl p-3 flex flex-col h-full shadow-sm md:hover:shadow-xl md:hover:-translate-y-1 transition-all duration-300"
      style={{
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 border-t border-gray-100 mt-2.5 pt-2.5">
        ...
        <button
          onClick={handleAddToCart}
          disabled={isAdded}
          className={`flex items-center gap-1 shrink-0 text-white text-xs font-medium px-3.5 py-1.5 rounded-full transition-all duration-200 ${
            isAdded ? "bg-green-500" : "bg-orange-500 hover:bg-orange-600"
          }`}
        >
          {isAdded ? (
